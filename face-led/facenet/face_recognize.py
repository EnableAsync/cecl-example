    ...
    with tf.Graph().as_default():
        with tf.Session() as sess:
            ...
            while cap.isOpened():
                ...
                mark, bounding_box, crop_image = load_and_align_data(
                    gray, 160, 44, pnet, rnet, onet)
                end1 = time.time()
                if mark:
                    start2 = time.time()
                    feed_dict = {images_placeholder: crop_image,
                    ...
                    if kk == ord('s'):
                        ...
                        compare_emb = np.insert(
                            compare_emb, len(compare_emb), emb, axis=0)
                    ...
                    if (len(compare_emb) != 0):
                        for i in range(temp_num):
                            ...
                            if (min_value > 0.9):
                                ...
                                fin_obj.append(
                                    compare_list[dist_list.index(min_value)])
                    ...
                    for rec_position in range(temp_num):
                        cv2.rectangle(img, (bounding_box[rec_position, 0]*2, bounding_box[rec_position, 1]*2),
                                      (bounding_box[rec_position, 2]*2,
                                       bounding_box[rec_position, 3]*2), (0, 255, 0),
                                      2, 8, 0)

                        cv2.putText(
                            img,
                            fin_obj[rec_position],
                            (bounding_box[rec_position, 0]*2,
                             bounding_box[rec_position, 1]*2),
                ...
                cv2.putText(img, "Faces: " + str(len(fin_obj)), (20, 140), font, 0.8, (0, 255, 0), 1,
                            cv2.LINE_AA)
                cv2.putText(img, "Q: Quit", (20, 450), font,
...
def publicInfo(stream):
    while stream.isOpened():
        ...
        if state == "unknown":
            ...
            state = "ON"
        client.publish('faceRecognize', state)

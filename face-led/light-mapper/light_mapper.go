...
import (
	...
	lightdriver "light-mapper/light_driver"
)

const (
	modelName                 = "LED-LIGHT-2"
	powerStatus               = "power-status"
	cameraStatus              = "faceRecognize"
...
func init() {
	...
	ClientOpts = HubClientInit(configFile.MQTTURL, "", "", "")
...
func subscribeCamera() {
	for {
		Camera_client = Client.Subscribe(cameraStatus, 0, OnCameraReceived)
		if Camera_client.Wait() && Camera_client.Error() != nil {
			glog.Error("subscribeCamera() Error in device twin result get  is ", Camera_client.Error())
		}
		if deviceTwinResult.Twin != nil {
			wg.Done()
			break
